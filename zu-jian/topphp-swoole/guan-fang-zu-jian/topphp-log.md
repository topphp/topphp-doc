日志记录和写入由`\think\Log`类完成，通常我们使用`Topphp\TopphpLog\Log`类进行静态调用。
...
可以添加多个日志通道，每个通道可以设置不同的日志类型。日志配置参数根据不同的日志类型有所区别，内置的日志类型包括：`File`、`Aliyun`，日志类型使用`type`参数配置即可。
...
还支持选择配置通道写入，第三个参数为channel通道，例如使用aliyun日志通道写入日志到阿里云日志：

```
Log::write("测试日志信息", "debug", "aliyun");
```
如果配置了默认记录通道'default' => env('log.channel', 'aliyun'),第三个通道参数就可以不传，直接正常调用：
...
> 为避免内存溢出，在命令行或Swoole服务环境下面执行的话日志信息会自动实时写入。
...
组件对系统的日志按照级别来分类记录，按照`PSR-3`日志规范，日志的级别从低到高依次为：`debug`,`info`,`notice`,`warning`,`error`,`critical`,`alert`,`emergency`，组件额外增加了一个`sql`日志级别仅用于记录`SQL`日志（并且仅当开启数据库调试模式有效）。
...
还集成了一个便于日志记录详细请求的参数，例如：

```
Log::write("日志信息", "日志级别(info)", "通道(aliyun)", "业务服务(service)", "业务操作(operate)");
// 通道传入空字符串表示使用配置设置的默认通道
Log::write("测试日志（带请求数据）", "info", "", "订单服务", "支付操作");
// 日志展示效果如下：
[2020-02-27T11:34:15+08:00][info] array (
  'service' => '订单服务',
  'operate' => '支付操作',
  'log-data' => '测试日志（带请求数据）',// 这是你记录日志的内容
  'headers' => 
  array (
  ),
  'method' => 'GET',
  'param' => '',// 此项为请求入参
  'url' => 'http://--no-configuration',// 请求url
)
```
八个日志级别快捷方法参数如下：
