...
还支持选择配置通道写入，例如使用aliyun日志通道写入日志到阿里云日志：

```
// 注意：考虑到业务需求，为方便开发者调用，仅 record 方法通道参数位于第三个
Log::record("测试日志信息", "debug", "aliyun");

// 其他写入日志方法都默认第五个参数为通道，所以如果是切通道写入的操作我们可以使用record，使用默认通道的都可以省略掉通道传参
Log::write("测试日志信息", "debug", "业务服务(service)", "业务操作(operate)", "aliyun");
```
如果配置了默认记录通道'default' => env('log.channel', 'aliyun'),第三个通道参数就可以不传，直接正常调用：

```
Log::record("测试日志信息", "error");
```
...
```
Log::write("日志信息", "日志级别(info)", "业务服务(service)", "业务操作(operate), "通道(aliyun)"");

// 通道传入空字符串或不传都表示使用配置设置的默认通道
...
```
八个日志级别和sql级别的日志快捷方法参数如下：

```
Log::alert("测试级别日志", "业务服务(service)", "业务操作(operate)", "通道(aliyun)");

// 下面是使用默认配置的通道进行写入
Log::alert("测试日志（带请求数据）", "订单服务", "支付操作");
```

事实上，你可以增加自定义的日志类型，自定义的日志类型不支持动态切换通道，例如：

```
Log::diy('这是一个自定义日志类型');

//需要动态切换通道使用自定义的日志类型的请如下调用
...
```
Log::info('日志信息{user}', "订单服务", "支付操作", "通道(aliyun)", ['user' => '流年']);
```
...
```
Log::handler('email')->info('一条测试日志');
Log::handler('socket')->error('记录错误日志');
```
...
表示如果是`error`日志和`sql`日志，会分别记录到指定的通道。同时你还需要在日志配置文件中，添加`email`和`sql`日志通道的配置。核心只有`File`与`Aliyun`日志类型，其它的可能需要自己扩展或者安装扩展。
