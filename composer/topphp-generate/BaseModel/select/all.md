...
> `$each`接收一个闭包函数，在闭包中，第一个参数`$item`即为每一条查询出来的数据集，`$each`回调会遍历循环每一条数据集，并执行闭包函数中的代码。省去了我们获取到数据再通过`foreach`来操作数据的操作。
...
如果你的`where`条件是一个`或者`的查询，尽管我们提供第五个参数`$isOr`是否是`or`查询，但是这样，一个方法调用就会很长，所以，我们还是推荐使用闭包的形式传入`or`条件：
